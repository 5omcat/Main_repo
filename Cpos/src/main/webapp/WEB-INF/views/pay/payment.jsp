	...
	<div class="row">
      ...
      <span></span>
		...
		<div class="col-sm-5 mt-1">
			...
			<button class="btn btn-success" data-toggle="modal"	data-target="#cashModal" id="cash">현금</button>
			<button class="btn btn-info" data-toggle="modal" data-target="#cardModal" id="card">카드</button>
	...
	<div class="modal fade" id="cashModal">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<!-- Modal body -->
				<div class="cash-modal-body mb-3" style="display: inline-block;text-align: center;">
					<table class="table">
						<thead>
							<tr class="table-primary">
								<th style="display:none">바코드</th>
				...
				<div class="modal-footer">
				  <span>총계</span><input type="text" class="sum" readonly size="2">
				  <span>수령액</span><input type="text" class="get" size="2">
				  <span>차액</span><input type="text" class="rest" readonly size="2">
  ...
  <div class="modal fade" id="cardModal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <!-- Modal body -->
        <div class="card-modal-body mb-3" style="display: inline-block;text-align: center;">
          <table class="table">
            <thead>
              <tr class="table-primary">
                <th style="display:none">바코드</th>
...
<script>
	$(function() {
	    ...
	    $(document).on("click", ".i", function() {
	      ...
	      let barcode = $(parentsUl).find("td:first-child").text();
	      let pname = $(parentsUl).find("td:nth-child(2)").text();
	      let category = $(parentsUl).find("td:nth-child(3)").text();
	      let get_price = $(parentsUl).find("td:nth-child(4)").text();
	      let sell_price = $(parentsUl).find("td:nth-child(5)").text();
	      let discount_rate = $(parentsUl).find("td:nth-child(6)").text();
	      addshoppinglist(barcode, pname, category, get_price, sell_price, discount_rate);
	    });
		
		/* 인벤토리 받아서, 중복체크하고 배열에 넣는 애 */
	  function addshoppinglist(barcode, pname, category, get_price, sell_price, discount_rate){
	    ...
	    if (dupleChk < 0) {
	      ...
	      getlist(barcode, pname, category, get_price, sell_price, discount_rate);
	    } else {
		...
		/* 쇼핑 리스트보여주는 애 */
		function getlist(barcode, pname, category, get_price, sell_price, discount_rate){
		...
		$(document).on("click", "button[name='minus']", function(){
			let nowNumVal = $(this).closest("ul").find("li:nth-child(7)").find("button:first-child").next().text();
			if (nowNumVal > 1) {
				nowNumVal --;
		  } else{
			  alert("수량은 1개 이상이여야 합니다.")
		  }
			$(this).closest("ul").find("li:nth-child(7)").find("button:first-child").next().text(" "+nowNumVal+" ");
			let nowSellPrice = $(this).closest("ul").find("li:nth-child(5)").text();
			console.log("nowSellPrice : " + nowSellPrice)
			let dc = $(this).closest("ul").find("li:nth-child(6)").text();
      console.log("dc : " + dc);
      nowTotalPrice = (nowNumVal * nowSellPrice * ( 1- parseInt(dc)*1/100 ));
			console.log("nowTotalPrice : " + nowTotalPrice);
			$(this).closest("ul").find("li:nth-child(8)").find("span:first-child").text(nowTotalPrice);
			caltotal();
	  });
		...
		$(document).on("click", "button[name='del']", function(){
         ...
         /* slist.forEach(function(item){
             console.log(
                          item.barcode + "," + item.pname + ", " + item.category + ", " +
                          item.get_price + ", " + item.sell_price + ", " + 
                          item.discount_rate);
           }); */
	  });
    ...
    function caltotal(){
      ...
      for (var i = 0; i < ultag.length; i++) {
        let li = ultag[i];
        let sell_price = li.childNodes[4].textContent;
        let dc = li.childNodes[5].textContent;
        let count = li.childNodes[6].childNodes[1].textContent;
    ...
    $(document).on("click", "#cash", function(){
    		console.log("모달눌렀을때");
    	      $(".cash-modal-body table:first-child").nextAll().remove();
    	      let ultag = $(document).find("#shoppinglist").find("ul");
    	      ...
    	      for (var i = 0; i < ultag.length; i++) {
    	        ...
    	        modaltag += '<li class="nav-item">' + li.childNodes[1].textContent + '</li>';
    	        modaltag += '<li class="nav-item" style="display:none">' + li.childNodes[2].textContent + '</li>';
    	        modaltag += '<li class="nav-item" style="display:none">' + li.childNodes[3].textContent + '</li>';
    	        modaltag += '<li class="nav-item">' + li.childNodes[4].textContent + '</li>';
    	        modaltag += '<li class="nav-item">' + li.childNodes[5].textContent + '</li>';
    	        modaltag += '<li class="nav-item">' +li.childNodes[6].childNodes[1].textContent+'</li>';
    	        modaltag += '<li class="nav-item">' +li.childNodes[7].textContent+ '</li></ul>';
    	        
    	        console.log("li.childNodes[4].textContent : " + li.childNodes[4].textContent);
    	        
    	        addpaylist("member", 
    	           li.childNodes[0].textContent, 
    	           li.childNodes[1].textContent, 
    	           li.childNodes[2].textContent, 
    	           li.childNodes[6].childNodes[1].textContent, 
    	          ...
    	          $(".cash-modal-body").append(modaltag);
    	      }
    ...
    $(document).on("click", "#card", function(){
      console.log("모달눌렀을때");
      $(".card-modal-body table:first-child").nextAll().remove();
      let ultag = $(document).find("#shoppinglist").find("ul");
      let receipt = createReceiptNo();
      for (var i = 0; i < ultag.length; i++) {
        let li = ultag[i];
        ...
        addpaylist("member", 
          ...
          $(".card-modal-body").append(modaltag);
      }
      let totalsum = $("input[name='totalsum']").val();
      $(".sum").val(totalsum);
      console.log("totallist.length : " + totallist.length);
